...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_replace_name", default=None, type=str, help="The path for your saved model")
    ...
    if args.save_replace_name:
        ori = args.save_replace_name.split('/')[0]
        tgt = args.save_replace_name.split('/')[1]
        ori_hf_path = hf_path
        hf_path = hf_path.replace(ori,tgt)
        print("target save path: ",hf_path)
        os.makedirs(hf_path,exist_ok=True)
        for filename in os.listdir(ori_hf_path):
            if filename.endswith(".json") or filename.endswith(".txt"):
                src_file = os.path.join(ori_hf_path, filename)
                dst_file = os.path.join(hf_path, filename)
