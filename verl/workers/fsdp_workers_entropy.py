...
from .config import ActorConfig, CriticConfig, FSDPConfig, ModelConfig, OptimConfig, WorkerConfig,RewardConfig
...
class FSDPWorker(Worker):
    def __init__(
        self,
        config: WorkerConfig,
        role: Literal["actor", "critic", "rollout","reward", "ref", "actor_rollout", "actor_rollout_ref"],
    ):
        super().__init__()
        self.config = config
        self.role = role
        ...
        self._has_reward = self.role == "reward"
        ...
        if self._has_reward:
            self._init_dist_mesh_reward(self.config.reward, "reward")
    ...
    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig, RewardConfig], role: Literal["actor", "critic","reward"]):
    ...
    def _init_dist_mesh_reward(self, config: Union[ActorConfig, CriticConfig, RewardConfig], role: Literal["actor", "critic","reward"]):
        world_size = dist.get_world_size()
        # create main device mesh
        fsdp_size = config.fsdp.fsdp_size
        if fsdp_size <= 0 or fsdp_size >= world_size:
    ...
    def _build_reward_model(self, model_config: ModelConfig):
        ...
        self.model_config = AutoConfig.from_pretrained(model_config.model_path, trust_remote_code=model_config.trust_remote_code)
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            ...
            num_labels=2,
            trust_remote_code=model_config.trust_remote_code,
            device_map=None,  # FSDP will manage devices
        ).to(torch.bfloat16)
        model.eval()
        auto_wrap_policy = get_fsdp_wrap_policy(model)
        sharding_strategy = ShardingStrategy.FULL_SHARD

        self.reward_module = FSDP(
            model,
            auto_wrap_policy=auto_wrap_policy,
            sharding_strategy=sharding_strategy,
            device_id=torch.cuda.current_device(),
    ...
    def extract_final_response(self,response_str: str) -> str:
        match = re.search(r"## Final Response\s*(.*)", response_str, re.DOTALL)
        if match:
    ...
    def init_model(self):
        ...
        if self._has_reward:
            self._build_reward_model(self.config.reward.model)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer: # NOTE 
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        data.meta_info["max_token_len"] = 8096
