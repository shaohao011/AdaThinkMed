...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    token_level_rewards = data.batch["token_level_rewards"] ## 只在最后一个有效位置
    response_mask = data.batch["response_mask"]
...
class RayPPOTrainer(UnifiedRayPPOTrainer):
    ...
    def __init__(
        ...
        if self.config.algorithm.use_rm:
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("old logp cal", timing_raw):
                    old_log_probs = self.actor_rollout_ref_wg.compute_log_prob(batch) ##
                    entropys = old_log_probs.batch["entropys"] # [64,4096]
                    response_masks = batch.batch["response_mask"] # [64,4096]  向右pad 0
                    # loss_agg_mode = "token-mean"  
                    loss_agg_mode = "seq-mean-token-mean"  
                    
                    entropy_loss = agg_loss(loss_mat=entropys, loss_mask=response_masks, loss_agg_mode=loss_agg_mode) # 1.3135
                    old_log_prob_metrics = {"actor/entropy_old_model": entropy_loss.detach().item()}
                    metrics.update(old_log_prob_metrics) ## name number
                    
                    batch = batch.union(old_log_probs) 
                    # prior reward calculation, we need to know the accuracy of each prompt
                ...
                if "token_level_scores" not in batch.batch:
                    with timer("reward", timing_raw):
                        ## NOTE combine reward calculation here
                        # breakpoint()
                ...
                with timer("adv", timing_raw):
                    ...
                    if not self.config.algorithm.use_kl_loss and self.use_reference_policy:
                        ...
                        batch.batch["token_level_rewards"] = batch.batch["token_level_scores"] ## same for each token

                    # compute advantages, executed on the driver process
                    # we need to fix here for shaping different difficulty
                ...
                # validate reward_fn is same with training
                if (
