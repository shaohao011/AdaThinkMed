...
from ..workers.fsdp_workers_entropy import FSDPWorker
from ..workers.reward import FunctionRewardManager
from . import core_algos
from .config import PPOConfig
from .core_algos import AdvantageEstimator, FixedKLController, KLController, compute_kl, get_kl_controller,agg_loss
...
def apply_kl_penalty(data: DataProto, kl_ctrl: KLController, kl_penalty="kl"):
    ...
    data.batch["token_level_rewards"] = token_level_scores - kl_ctrl.kl_coef * kld ## 将 KL 作用到 reward 上面
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(token_level_rewards, response_mask, index) # 归一化并广播到每一个有效token上面
...
class UnifiedRayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        all_subset_lengths = defaultdict(list)  
        sample_subset_length_pairs = []         
        ...
        for subset, lengths in all_subset_lengths.items():
            ...
            half = len(sorted_lengths) // 2
            short_set = set(sorted_lengths[:half])
            long_set = set(sorted_lengths[half:])
            subset_half_split[subset] = (short_set, long_set)

        for subset, length in sample_subset_length_pairs:
            short_set, long_set = subset_half_split[subset]
            if length in short_set:
                length_group = "short"
            else:
                length_group = "long"
